	...
	def private highlightFileToken(ILeafNode leaf, IHighlightedPositionAcceptor acceptor) {
		...
		if(obj instanceof XStringLiteral && obj.eContainer instanceof XBinaryOperation) {
			...
			for(seg:segments) {
				...
				if(file == null)
					acceptor.addPosition(seg.offset, seg.name.length, CradleHighlightingConfiguration::FILE_SEGMENT_UNKNOWN)
				else if(file.exists)
					acceptor.addPosition(seg.offset, seg.name.length, CradleHighlightingConfiguration::FILE_SEGMENT_PRESEND)
				else
					acceptor.addPosition(seg.offset, seg.name.length, CradleHighlightingConfiguration::FILE_SEGMENT_ABSENT) 
			}
		}
	}
