...
import cradle.Mwe2IfNeeded.Params;
import java.io.File;
...
import org.xtext.cradle.lib.Literals.DigestInit;
...
public class Mwe2IfNeeded {
    ...
    try {
      for(String task:tasks) {
        ...
        if("runMwe2IfNeeded".equals(task))
          executeRunMwe2IfNeeded(parameter);
        index++;
  ...
  public static void runMwe2IfNeeded(final Procedure1<Params> paramInitializer) {
    Params p = new Params();
    paramInitializer.apply(p);
    executeRunMwe2IfNeeded(p);
  }
  
  protected static void executeRunMwe2IfNeeded(final Params it) {
